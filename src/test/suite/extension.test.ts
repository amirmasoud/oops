...
suite('Extension Test Suite', () => {
	...
	test('Should change file to folder', () => {
		...
		fs.rename(testFolderLocation + 'oops_folder/.gitkeep', testFolderLocation + '.gitkeep', () => {
			Oops.fileToFolder(uri).then(() => {
				assert.ok(fs.lstatSync(uri.fsPath).isDirectory());
			});
			...
			fs.rename(testFolderLocation + '.gitkeep', testFolderLocation + 'oops_folder/.gitkeep', () => { });
		});
	});
